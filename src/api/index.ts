...
export const getHotels = async ({city, checkIn, checkOut}: HotelsParams) => {
    const {data} = await
        axios
            .get<IHotels[]>
            (`${process.env.REACT_APP_HOTELS_API}location=${city}&lang=ru&currency=rub&checkIn=${checkIn}&checkOut=${checkOut}&limit=10`)
    return data
}